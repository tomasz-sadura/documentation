...
- A service account with full access to the external cluster must be available.
  ...
  topic location is set to `target`. You need describe/describe-configs ACLs for
  the connector to read topic configs on the source cluster and create the topics
...
| **TopicAuthorizationException: Topic authorization failed** | Confirm the service account for source cluster contains describe/describe-configs ACLs. |
